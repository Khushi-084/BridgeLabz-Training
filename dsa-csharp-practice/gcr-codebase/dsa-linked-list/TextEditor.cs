...
class UndoRedoTextEditor{
    ...
    static void AddState(string text){
        ...
        // If first state
        if (head == null){
            head = tail = current = newNode;     // Adding at the beginning
            size = 1;
            return;
        }
        // Remove redo states
        if (current != tail){
            Node temp = current.Next;       // Traversing the list 
            while (temp != null){           // Removing redo states
                Node next = temp.Next;      // Adding at the end
                temp = next;                // Adding at the end
    ...
    // Redo operation
    static void Redo(){
        if (current != null && current.Next != null){      // Traversing the list 
