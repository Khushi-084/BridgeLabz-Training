...
class SocialMediaConnections{
    ...
    static Node FindUserByID(int id){
        ...
        while (temp != null){      // Traversing the list 
            if (temp.Id == id){       // If the user is found
                return temp;
            }
            temp = temp.Next;       // If the user is not found
        }
        return null;
    }
    ...
    static void AddFriend(int id1, int id2){
        ...
        if (user1 == null || user2 == null){        // If the user is not found
            Console.WriteLine("not found.");
            return;
        }
        user1.Friends[user1.FriendsCounts++] = id2;      // Adding the friend
    ...
    static void RemoveFriend(Node user, int friendID){
        for (int i = 0; i < user.FriendsCounts; i++){         // Traversing the list
            if (user.Friends[i] == friendID){                 // If the friend is found
                for (int j = i; j < user.FriendsCounts - 1; j++){        // Removing the friend
                    user.Friends[j] = user.Friends[j + 1];
                }
                user.FriendsCounts--;            // Decreasing the number of friends
                return;
            }
        }
    }
    ...
    static void FindMutualFriends(int id1, int id2){
        // Find the users by ID 
        Node user1 = FindUserByID(id1);        // If the user is not found
        Node user2 = FindUserByID(id2);
        ...
        bool found = false;
        for (int i = 0; i < user1.FriendsCounts; i++){      // Traversing the list
    ...
    static void SearchByName(string name){
        ...
        bool found = false;
        while (temp != null){            // Traversing the list
            if (temp.Name == name){      // If the user is found
                DisplayUser(temp);
                found = true;
            }
            temp = temp.Next;         // If the user is not found
