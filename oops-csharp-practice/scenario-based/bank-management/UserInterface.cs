...
public class UserInterface{
    ...
    public UserInterface(AccountDatabase db){
        database = db;
    }
    ...
    private void Register(){
        ...
        double deposit;
        while (!double.Parse(Console.ReadLine(), out deposit) || deposit < 0){
            Console.Write("Invalid amount! Enter again: ");
        }
        BankAccount account = database.AddAccount(name, password, deposit);
        if (account != null){
    ...
    private void Login(){
        ...
        int accNum;
        while (!int.Parse(Console.ReadLine(), out accNum)){
        ...
        while (true){
            ...
            switch (choice){
                case "1":
                    ...
                    double dep;
                    while (!double.Parse(Console.ReadLine(), out dep) || dep <= 0){
                        Console.Write("Invalid amount! Enter again: ");
                    }
                    account.Deposit(dep);
                    break;
                case "2":
                    ...
                    double wit;
                    while (!double.Parse(Console.ReadLine(), out wit) || wit <= 0){
                        Console.Write("Invalid amount! Enter again: ");
                    }
